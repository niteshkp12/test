...
public class CustomMap<K, V> extends AbstractMap<K, V> implements Map<K, V>,
	 ...
	 */
	public static final int ALTERNATIVE_HASHING_THRESHOLD_DEFAULT = Integer.MAX_VALUE;
	...
	private static class Holder {
		...
		static {
			...
			/*
			 * try { UNSAFE = sun.misc.Unsafe.getUnsafe(); HASHSEED_OFFSET =
			 * UNSAFE.objectFieldOffset(
			 * CustomMap.class.getDeclaredField("hashSeed")); } catch
			 * (NoSuchFieldException | SecurityException e) { throw new
			 * Error("Failed to record hashSeed offset", e); }
			 */
		}
	}
	...
	public V put(K key, V value) {
		...
		for (Entry<K, V> e = table[i]; e != null; e = e.next) {
			System.err.println("Table key: " + e.key + ", value: " + e.value);
		}
	 ...
	 */
	public Object clone() {
		CustomMap<K, V> result = null;
		try {
			result = (CustomMap<K, V>) super.clone();
	...
	static class Entry<K, V> implements Map.Entry<K, V> {
		 ...
		 * HashMap.
		 */
		void recordAccess(CustomMap<K, V> m) {
		}

		/**
		 * This method is invoked whenever the entry is removed from the table.
		 */
		void recordRemoval(CustomMap<K, V> m) {
		}
	}
	...
	private abstract class HashIterator<E> implements Iterator<E> {
		...
		public void remove() {
			...
			current = null;
			CustomMap.this.removeEntryForKey(k);
			expectedModCount = modCount;
		}
	}
	...
	private final class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return CustomMap.this.removeEntryForKey(o) != null;
		}
